...
pub use self::{config::Config, token::Token};
use crate::{
...
impl Bot
{
	...
	pub async fn new(config: Config, database: Database) -> Result<Self, DiscordError>
	{
		...
		let framework = poise::Framework::builder()
			.options(framework_options(config.owners.iter().copied()))
			.setup({
		...
		let client = serenity::Client::builder(token.as_str(), gateway_intents())
			.framework(framework)
			.activity(ActivityData::custom("(͡ ͡° ͜ つ ͡͡°)"))
			.await?;

		Ok(Self { tx, rx, config, database, client })
	}
			...
			select! {
				...
				Some(message) = self.rx.recv() => match message {
					BotMessage::AssignMapperRole { user_id } => {
						self.assign_mapper_role(user_id).await?;
					},
					BotMessage::RevokeMapperRole { user_id } => {
						self.revoke_mapper_role(user_id).await?;
					},
				},

				result = self.client.start() => match result {
					Ok(()) => {
						tracing::info!("discord bot shutting down");
						return Ok(());
					},
	...
	async fn assign_mapper_role(&mut self, user_id: UserId) -> Result<(), DiscordError>
	{
		...
		if let Some(Some(user_id)) = discord_user_id {
			if let Ok(member) = self.config.guild_id.member(&self.client.http, user_id).await {
				member.add_role(&self.client.http, mapper_role_id).await?;
	...
	#[tracing::instrument(skip(self), err)]
	async fn revoke_mapper_role(&mut self, user_id: UserId) -> Result<(), DiscordError>
	{
		let Some(mapper_role_id) = self.config.roles.mapper else {
			tracing::warn!("no mapper role configured");
			return Ok(());
		...
		if let Some(Some(user_id)) = discord_user_id {
			if let Ok(member) = self.config.guild_id.member(&self.client.http, user_id).await {
				member.remove_role(&self.client.http, mapper_role_id).await?;
