...
use futures_util::{Stream, StreamExt as _, TryFutureExt, TryStreamExt, stream::FuturesUnordered};
use serde::Serialize;
use sqlx::Row;
...
use crate::{
	...
	time::Timestamp,
	users::{self, InvalidUsername, UserId, Username},
};
...
#[tracing::instrument(skip(conn, steam_api_client), ret(level = "debug"), err)]
#[builder(finish_fn = exec)]
pub async fn create<I, CourseMappers>(
	#[builder(start_fn)] workshop_id: WorkshopId,
	#[builder(finish_fn)] conn: &mut DatabaseConnection<'_, '_>,
	#[builder(finish_fn)] steam_api_client: &steam::api::Client,
	name: MapName,
	...
	CourseMappers: IntoIterator<Item = UserId> + fmt::Debug,
{
	...
	async {
		for course in courses {
			let course_id = create_course(map_id)
				.course(course)
				.exec(&mut *conn, steam_api_client)
				.await?;
	...
	CourseMappers: IntoIterator<Item = UserId> + fmt::Debug,
{
	...
	async {
		let mut mappers = FuturesUnordered::from_iter(course.mappers.into_iter().map(|user_id| {
		...
		while let Some(mapper) = mappers.try_next().await? {
			...
			let mapper_name = mapper
				.name
				.parse::<Username>()
				.map_err(|err| CreateMapError::InvalidMapperName { id: mapper_id, error: err })?;

			if let Ok(()) = users::create(mapper_id).name(mapper_name).exec(&mut *conn).await {
				tracing::debug!(
					id = %mapper_id,
			...
			create_course_mapper(course_id).user_id(mapper_id).exec(&mut *conn).await?;

			tracing::debug!(id = %mapper_id, "created course mapper");
		}

		Result::<(), CreateMapError>::Ok(())
	}
	.instrument(tracing::debug_span!("create_course_mappers", %course_id))
	.await?;
