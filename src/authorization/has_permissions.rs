...
// NOTE: ideally this const generic would be `Permissions` instead of `u32`, but
// as of writing       this, Rust does not allow it
...
impl<const PERMS: u32> AuthorizeSession for HasPermissions<PERMS>
{
	...
	) -> Result<()>
	{
		...
		tracing::Span::current()
			.record("required_permissions", format_args!("{required_permissions}"));
