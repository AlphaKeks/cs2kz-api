...
/// A type for working with [Valve's SteamIDs].
///
/// This type is a thin wrapper around a 64-bit integer, as described by Valve.
/// It has a bunch of useful methods and trait implementations that can be used
/// to encode, decode, and format SteamIDs.
///
/// [Valve's SteamIDs]: https://developer.valvesoftware.com/wiki/SteamID
#[repr(transparent)]
...
impl SteamID
{
	...
	/// # Safety
	///
	/// The caller must ensure that `value` is within
...
/// Converting to a [`SteamID`] failed, because the value was out of range for a
/// valid `SteamID`.
