...
CREATE TABLE IF NOT EXISTS RatingsToRecalculate (
  player_id INT8 UNSIGNED NOT NULL PRIMARY KEY REFERENCES Players(id),
  priority INT8 UNSIGNED NOT NULL,
  INDEX IDX_PlayersToRecalculate_priority (priority)
);

CREATE OR REPLACE TRIGGER schedule_ratings_recalc_nub
  AFTER INSERT ON BestNubRecords FOR EACH ROW
  INSERT INTO RatingsToRecalculate
  SELECT DISTINCT player_id, 1
  FROM (
    SELECT
      RANK() OVER (
...
CREATE OR REPLACE TRIGGER schedule_ratings_recalc_pro
  AFTER INSERT ON BestProRecords FOR EACH ROW
  INSERT INTO RatingsToRecalculate
  SELECT DISTINCT player_id, 1
  FROM (
