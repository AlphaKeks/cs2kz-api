...
//!
//! This crate provides the [`ProblemDetails`] type, which models an HTTP
//! response that is compliant with [RFC 7807]. It is intended to be used with
//! the `axum` crate, which requires the `axum` feature to be enabled.
//! [`ProblemDetails`] will then implement `axum::response::IntoResponse`.
//!
//! ## Usage
//!
//! First, you must define your own [`ProblemType`]. Because every API has
//! different problems, [`ProblemDetails`] is generic over this field. You can
//! then create an instance of [`ProblemDetails`] using
//! [`ProblemDetails::new()`] and add more information to it using the available
//! methods. You can then turn it into an [`http::Response`] via [`From`] /
//! [`Into`].
//!
...
impl<T> ProblemDetails<T> {
	...
	///
	/// [problem type]: ProblemDetails::problem_type()
	pub const fn extension_members(&self) -> &ExtensionMembers {
