...
pub trait ProblemType: Sized
{
	...
	///
	/// The fragment produced by [`ProblemType::fragment()`] should always parse back into the
	/// same problem type that it was produced from.
...
#[derive(Debug)]
pub(crate) struct SerializeProblemType<'a, T>
where
	T: ProblemType,
{
	problem_type: &'a T,
...
impl<'a, T> SerializeProblemType<'a, T>
where
	T: ProblemType,
{
	pub(crate) fn new(problem_type: &'a T) -> Self
	{
		Self { problem_type }
	}
}
	...
	T: ProblemType,
{
		...
		S: Serializer,
	{
		...
		uri.push_str(self.problem_type.fragment());
		uri.serialize(serializer)
	}
}

#[derive(Debug)]
pub(crate) struct DeserializeProblemType<T>(pub T)
where
	T: ProblemType;
		...
		D: Deserializer<'de>,
	{
		...
		if base_uri != T::base_uri() {
			return Err(de::Error::invalid_value(
				Unexpected::Str(base_uri),
				&"correct base uri",
			));
		}

		T::parse_fragment(fragment)
			.map(Self)
