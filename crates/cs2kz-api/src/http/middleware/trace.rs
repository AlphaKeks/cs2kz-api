...
/// Creates a [`tower::Layer`], which produces a middleware that will emit
/// tracing spans & events for every HTTP request.
///
/// The `include_headers` parameter can be used to control whether
/// request/response headers should be included in the traces.
pub fn layer<B>(
...
> {
	let make_span =
		move |req: &http::Request<B>| -> tracing::Span { make_span(req, include_headers) };
...
///    1. Another middleware runs before it, namely `RequestIdLayer`. It
///       generates a unique ID that is inserted into both the request's headers
///       and extensions. We want to include this ID in traces even if headers
///       are not recorded, so `make_span` will extract it.
///    2. The `TraceLayer` using this function is part of a service stack that
///       is eventually passed to an `axum::Router` that is turned into a
///       `tower::Service` via
///       `axum::Router::into_make_service_with_connect_info::<SocketAddr>()`.
///       This will ensure a `ConnectInfo` value is inserted into the request
///       extensions. This value contains the client address and port of the TCP
///       connection.
///
/// If any of the above assumptions are not true, this function will panic.
fn make_span<B>(request: &http::Request<B>, include_headers: bool) -> tracing::Span {
...
///
/// What qualifies as a "failure" is determined by `ServerErrorsFailureClass`,
/// but generally it is a `500 Internal Server Error` status code.
