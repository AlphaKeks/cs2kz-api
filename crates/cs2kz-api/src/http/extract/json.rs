...
///
/// This type also implements [`IntoResponse`], which means it can be returned from handlers.
///
...
/// Checks if the given `headers` contain a `Content-Type` header with a JSON-related value.
